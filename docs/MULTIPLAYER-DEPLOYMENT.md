...
After changing **simulation tick rate** (`TICK_RATE` in `server.js`), you must align the client (`NET_SYNC_HZ` in `index.html`) — so prefer tuning **AOI and player cap** before changing tick rate.
